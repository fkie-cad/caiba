    ...
    struct CAN_XR_MAC *mac, unsigned long ts, int input_unit)
{
	...
	*/
    ...
    case CAN_XR_MAC_RX_FSM_ERROR:
        mac->state.tx_fsm_state = CAN_XR_MAC_TX_FSM_IDLE;
        CAN_XR_PCS_Data_Req(mac->pcs, 0);
        mac->state.field_bits = 3; /* first two bits of error was already sent */
    ...
    struct CAN_XR_PCS *pcs)
{
    ...
    /* This does nothing but turning off a pin that I used for signaling.
     * However, without this command, the sender does not correctly
     * receive messages from the receiver. If you don't ask, nobody
     * gets hurt...*/
