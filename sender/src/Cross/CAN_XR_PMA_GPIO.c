...
static void init_gpio(void)
{
    /* Set FIO0DIR<5> = 1, FIO0DIR<4> = 0,
         GPIO Port 0.5 (MBED PIN29) must be an output, 0.4 an input (MBED PIN30).
...
void CAN_XR_PMA_GPIO_Set_App_NodeClock_Ind(
    struct CAN_XR_PMA *pma, CAN_XR_PMA_App_NodeClock_Ind_t app_nodeclock_ind)
{
    ...
    while(1)
    {
        ...
        */
        ...
        if (pma->state.gpio.app_nodeclock_ind) {
            if (pma->state.gpio.app_nodeclock_ind(pma->pcs)) {
                /* need to wait for synchronization again here, since the MAC computation in app_nodeclock_ind takes some
             * time. This leads to a transmission of the registered message in high bitrate and to problems at receiving
             * nodes */
                x = read_ts() - 1;
            }
        }
