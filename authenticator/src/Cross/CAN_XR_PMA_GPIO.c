...
/* Set to HIGH --- recessive for the SN65HVD23x */
#define gpio_tx_rec()  (FIO0SET = PORT_0_5_MASK)

/* Set to LOW --- dominant for the SN65HVD23x */
#define gpio_tx_dom()  (FIO0CLR = PORT_0_5_MASK)

/* Overwrite dominant to recessive --- set LOW to SN65HVD23x CAN Transceiver connected inverted */
#define gpio_ow_rec()    (FIO0CLR = PORT_0_10_MASK)

/* Stop overwriting --- set HIGH to SN65HVD23x connected inverted */
#define gpio_stop_ow_rec()   (FIO0SET = PORT_0_10_MASK)
...
/* Read back value of forcing pin --- 0: dominant, 1: recessive */
#define gpio_ow_pin()  ((FIO0PIN & PORT_0_10_MASK) ? 1 : 0)
...
{
    /* Set FIO0DIR<5> = 1, FIO0DIR<4> = 0, FIODIR<10> = 1
         GPIO Port 0.5 and 0.10 (MBED PIN29, PIN28) must be an output, 0.4 an input (MBED PIN30).

       Set PINMODE0<9:8> = 10,
         GPIO Port 0.4 must have neither pull-up nor pull-down.
	 PINMODE0 is unused for outputs.

       Set PINMODE_OD0<5> = 0, PINMODE_OD0<10> = 0
         GPIO Port 0.5 and 0.10 must not be open-drain.
	 PINMODE_OD0 is unused for inputs.

       Set output to recessive to not perturb the bus.

       Set PINSEL0<11:10> = 00, PINSEL0<9:8> = 00, PINSEL0<21:20> = 00, this configures
         P0.5, P0.4 and P0.10 as GPIO pins.  This is the last action to avoid
         connecting to the physical pins GPIO signals not configured
         in the right way.
    */
    FIO0DIR     = (FIO0DIR     & ~0x00000430) | 0x00000420;
    PINMODE0    = (PINMODE0    & ~0x00000300) | 0x00000200;
    PINMODE_OD0 = (PINMODE_OD0 & ~0x00000420) | 0x00000000;
    gpio_tx_rec();
    gpio_stop_ow_rec();
    PINSEL0     = (PINSEL0     & ~0x00300F00) | 0x00000000;

    TRACE(0, ">>> gpio_tx_pin/rx_pin after init: %d/%d",
	  gpio_tx_pin(), gpio_rx_pin(), gpio_ow_pin());
}
...
static void data_mac_req(struct CAN_XR_PMA *pma, int bus_level)
{
    if (!gpio_rx_pin() && bus_level) {  // bus is dominant, recessive required
        gpio_ow_rec();
    } else if(gpio_rx_pin() == 1 && bus_level == 0) {    // bus is recessive, dominant required
        gpio_tx_dom();
    }
}
...
void CAN_XR_PMA_GPIO_Init(struct CAN_XR_PMA *pma, int prescaler)
{
    ...
    pma->primitives.data_mac_req = data_mac_req;
    pma->primitives.tx_reset = tx_reset;    /* called to reset both transceivers */
