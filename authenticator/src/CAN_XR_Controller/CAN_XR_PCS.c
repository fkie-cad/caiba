...
static void init_state(struct CAN_XR_PCS *pcs)
{
    ...
    */
    ...
    /* Enabled while controller handles data mac.
     * Enables reading signal during prop_seg
     */
    pcs->state.fast_pass = 0;
    pcs->state.res_fast_pass = 0;

    /* compensation of sync at sender and receiver for special cases during
     * overwriting
     */
    pcs->state.sync_compensation = 0;
}
...
static void data_req_stop(struct CAN_XR_PCS *pcs)
{
    CAN_XR_PMA_Tx_Reset(pcs->pma);
}
    ...
    struct CAN_XR_PCS *pcs, unsigned long ts, int bus_level)
{
    ...
    if (pcs->state.fast_pass)
    {
        // Resetting here seems to be better than in last time quantum
        if (pcs->state.quantum_m_cnt == 0) {
            CAN_XR_PMA_Tx_Reset(pcs->pma);
        }
        if (pcs->state.quantum_m_cnt == pcs->parameters.sync_seg) /* one after sync */
        {
            pcs->state.prev_bus_level = bus_level;
            pcs -> primitives.data_ind(pcs->mac, ts, bus_level);    /* handle state of MAC layer earlier here */
            CAN_XR_PMA_Data_Mac_Req(pcs->pma, pcs->state.output_unit_buf);

            /*
             * If the previous bus level was recessive, the sender transmits recessive and the authenticator overwrites
             * it dominant, an edge will be recognized one quantum after sync and it will be used for synchronization.
             * Thus, the authenticator has to overwrite one quantum longer.
             */
            if (pcs->state.prev_sample && !pcs->state.output_unit_buf && pcs->state.prev_bus_level)
            {
                pcs->state.sync_compensation = 1;
            }
            pcs->state.prev_sample = pcs->state.output_unit_buf;
        }

        else if(pcs->state.quantum_m_cnt == pcs->parameters.sync_seg + 1) {
            pcs->state.quantum_m_cnt -= pcs->state.sync_compensation;
            pcs->state.sync_compensation = 0;
        }

        else if(pcs->state.quantum_m_cnt >= pcs->state.quanta_per_bit - 1) {
            /* reset both transceiver to recessive state to avoid overwriting
              of next signal.
             */

            if (pcs->state.res_fast_pass)
            {
                CAN_XR_PMA_Tx_Reset(pcs->pma);
                pcs->state.fast_pass = 0;
                pcs->state.res_fast_pass = 0;
                pcs->state.prev_bus_level = pcs->state.prev_sample;
            }
        }
    }
    else
    ...
    pcs->state.quantum_m_cnt = (pcs->state.quantum_m_cnt + 1) % pcs->state.quanta_per_bit;
    ...
    struct CAN_XR_PMA *pma)
{
    ...
    pcs->primitives.data_req_stop = data_req_stop;
...
void CAN_XR_PCS_Data_Req_Stop(struct CAN_XR_PCS *pcs)
{
    if(pcs->primitives.data_req_stop)
    {
        pcs->primitives.data_req_stop(pcs);
    }
}
...
void CAN_XR_PCS_Set_Fast_Pass(
        struct CAN_XR_PCS *pcs, int fast_pass)
{
    pcs->state.fast_pass = fast_pass;
}

void CAN_XR_PCS_Reset_Fast_Pass(struct CAN_XR_PCS *pcs)
{
    pcs->state.res_fast_pass = 1;
}
