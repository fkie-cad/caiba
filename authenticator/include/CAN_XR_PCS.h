...
struct CAN_XR_PCS_State
{
    ...
    int prev_sample; /* Bus @ previous sample point for edge detection and sync compensation */
    ...
    uint8_t fast_pass;  /* Set by MAC to allow/forbid passing of bus state during prop_state */
    /* we need an additional flag to reset the fast_pass after a quantum. Otherwise, the fast_pass flag is reset
     * within the first half of the quantum. Then PCS and MAC is half a quantum too fast
     */
    uint8_t res_fast_pass; /* reset fast_pass when quantum ended */
    uint8_t sync_compensation;
};
...
typedef void (* CAN_XR_PCS_Data_Req_Stop_t)(
        struct CAN_XR_PCS *this);
...
struct CAN_XR_PCS_Primitives
{
    ...
    CAN_XR_PCS_Data_Req_Stop_t data_req_stop;
};
...
/* Invoke the data_req primitive in 'pcs'. Forces the value on the bus and thus can overwrite signals */
void CAN_XR_PCS_Data_Req(struct CAN_XR_PCS *pcs, int output_unit);

/* Invokes the data_req primitive in 'pcs' to stop transmission. Sets both transceiver to recessive which can be overwritten by other signals */
...
/* Dis-/Enables PCS to pass bit to MAC layer during prop_seg
 */
void CAN_XR_PCS_Set_Fast_Pass(
        struct CAN_XR_PCS *pcs, int fast_pass);

void CAN_XR_PCS_Reset_Fast_Pass(
        struct CAN_XR_PCS *pcs);
